...
import { createTaskAsync, fetchTasks, deleteTaskAsync, updateTaskStatus } from "@/store/slices/taskSlice";
...
function TaskModal({ onClose, onSave, employees, isAdmin }: { onClose: () => void; onSave: (data: any, sendEmail: boolean, empEmail: string) => void; employees: Employee[]; isAdmin: boolean }) {
  const [form, setForm] = useState({ title:"", description:"", assigneeRole: "EMPLOYEE", employeeId: "", priority:"medium", deadline:"", territory:"", recurring: false, recurrenceRule: "WEEKLY" });
  const [sendEmail, setSendEmail] = useState(true);
  const set = (k: string, v: any) => setForm(f => ({ ...f, [k]: v }));
  
  const filteredForRole = employees.filter(e => {
    if (!isAdmin) return true;
    if (form.assigneeRole === "MANAGER") return e.role === "MANAGER";
    return e.role !== "MANAGER";
  });

  useEffect(() => {
    if (!form.employeeId || !filteredForRole.find(e => e.id === form.employeeId)) {
      set("employeeId", filteredForRole[0]?.id || "");
    }
  }, [form.assigneeRole, employees]);
      ...
      <div className="modal-box" style={{ maxWidth:"520px" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:"14px" }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px" }}>
            {isAdmin && (
              <div>
                <label style={{ fontSize:"12px",fontWeight:600,color:"var(--text-secondary)",display:"block",marginBottom:"6px" }}>Assign To Role *</label>
                <select className="input" value={form.assigneeRole} onChange={e=>set("assigneeRole",e.target.value)}>
                  <option value="EMPLOYEE">Employee (Field / Office)</option>
                  <option value="MANAGER">Manager</option>
                </select>
              </div>
            )}
            <div style={{ gridColumn: isAdmin ? "auto" : "1 / span 2" }}>
              <label style={{ fontSize:"12px",fontWeight:600,color:"var(--text-secondary)",display:"block",marginBottom:"6px" }}>Select User *</label>
              <select className="input" value={form.employeeId} onChange={e=>set("employeeId",e.target.value)}>
                {filteredForRole.map(e=><option key={e.id} value={e.id}>{e.name} {isAdmin ? `(${e.role})` : ""}</option>)}
              </select>
            </div>
          </div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 4, padding: "8px 12px", background: "var(--bg-secondary)", borderRadius: 8, border: "1px solid var(--border)" }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 600, color: "var(--text-primary)", cursor: "pointer", flex: 1 }}>
              <input type="checkbox" checked={form.recurring} onChange={(e) => set("recurring", e.target.checked)} style={{ width: 16, height: 16, accentColor: "var(--accent-blue)" }} />
              Repeated / Recurring Task
            </label>
            {form.recurring && (
              <select className="input" style={{ width: "150px", padding: "6px 10px", fontSize: 13 }} value={form.recurrenceRule} onChange={(e) => set("recurrenceRule", e.target.value)}>
...
export default function TasksView() {
  ...
  return (
    <div>
      ...
      {modal && <TaskModal isAdmin={user?.role === "ADMIN"} employees={filteredEmployees} onClose={()=>setModal(false)} onSave={(data, sendEmail, empEmail)=>{ 
